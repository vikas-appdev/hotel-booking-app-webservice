...
public interface HotelRepository extends JpaRepository<Hotel, Integer> {
	...
	@Query(value="select h.*, (6371 * acos(cos(radians(:latitude)) * cos(radians(h.latitude)) * cos(radians(h.longitude) - "
			+ "radians(:longitude)) + sin(radians(:latitude)) * sin(radians(h.latitude)) )) as distance from hotel as h JOIN room as r ON h.id=r.hotel_id JOIN "
			+ "booking as b ON r.id=b.room_id WHERE (:fromTime < b.from_time || :fromTime > b.upto_time) && (:uptoTime < b.from_time || :uptoTime > b.upto_time) "
	...
	public List<Hotel> findHotelByLocation(@Param("latitude") final double latitude, 
			...
			@Param("fromTime") final LocalDateTime fromTime, @Param("uptoTime") final LocalDateTime uptoTime, 
